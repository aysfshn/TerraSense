...
def main():
    ...
    print("\n=== 2) Arsa Ekleme İsteği Gönderiliyor... ===")
    ...
    # 3) Arsa listeleme
    print("\n=== 3) Arsa Listeleme İsteği Gönderiliyor... ===")
    r = requests.get(f"{BASE_URL}/arazi/{arsa_id}", headers=headers)
    print("Cevap:", r.status_code, r.json())
    
    
    # 4) Arsa tavsiyesi alma (Token bilgisi de gönderiliyor)
    print("\n=== 4) Arsa Tavsiyesi Alma İsteği Gönderiliyor... ===")
    r = requests.get(f"{BASE_URL}/arazi/tavsiye/{arsa_id}", headers=headers)
    print("Cevap:", r.status_code, r.json())
